...
solucao(ListaSolucao) :-
   ...
   exatamenteADireita(turista(_, _, _, _, 45, _), turista(_, _, _, _, 31, _), ListaSolucao),

   % O Alemão está exatamente à esquerda do turista de Vermelho.
   exatamenteAEsquerda(turista(_, alemao, _, _, _, _), turista(vermelha, _, _, _, _, _), ListaSolucao),

   % O turista de 36 anos está exatamente à esquerda do turista que gosta de Leite.
   exatamenteAEsquerda(turista(_, _, _, _, 36, _), turista(_, _, leite, _, _, _), ListaSolucao),
   ...
   exatamenteADireita(turista(_, _, _, _, 28, _), turista(_, _, _, 25, _, _), ListaSolucao),

   % O turista de Verde está em algum lugar entre o Espanhol e o turista de Branco, nessa ordem;
   entre(turista(verde, _, _, _, _, _), turista(_, espanhol, _, _, _, _), turista(branca, _, _, _, _, _), ListaSolucao),
   ...
   exatamenteAEsquerda(turista(_, _, _, _, _, esposa), turista(_, _, leite, _, _, _), ListaSolucao),

   % Os turistas das camisas amarela e vermelha estão lado a lado.
   aoLado(turista(amarela, _, _, _, _, _), turista(vermelha, _, _, _, _, _), ListaSolucao),
